...
class MultiLayerMM(ModelsParentClass):
    ...
    def vertical_filter_model(self):
        ...
        x = tf.keras.layers.Conv2D(128, [3, 3],
                                   ...
                                   kernel_regularizer=tf.keras.regularizers.l2(1e-5),
                                   name='conv2')(x)
        x = tf.keras.layers.BatchNormalization(axis=channel_axis, name='bn2')(x)
        x = tf.keras.layers.ELU()(x)
        x = tf.keras.layers.MaxPool2D(pool_size=[2,2], strides=(2, 2),name='pool2',data_format='channels_first')(x)
        ...
        x = tf.keras.layers.Conv2D(128, [3, 3],
                                   ...
                                   kernel_regularizer=tf.keras.regularizers.l2(1e-5),
                                   name='conv3')(x)
        x = tf.keras.layers.BatchNormalization(axis=channel_axis, name='bn3')(x)
        x = tf.keras.layers.ELU()(x)
        x = tf.keras.layers.MaxPool2D(pool_size=2, strides=(2, 2),name='pool3',data_format='channels_first')(x)

        x = tf.keras.layers.Flatten(name='flatten')(x)
        x = tf.keras.layers.Dropout(0.5)(x)
        x = tf.keras.layers.Dense(256, kernel_initializer='he_normal', kernel_regularizer=tf.keras.regularizers.l2(1e-5), name='fc1')(x)
        x = tf.keras.layers.ELU()(x)
        x = tf.keras.layers.Dropout(0.5)(x)
